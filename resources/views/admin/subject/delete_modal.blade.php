...
<div id="deleteModalSubject" 
    ...
    <div class="relative p-4 w-full max-w-md">
        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800">
            ...
            <!-- Close Button -->
            <button type="button"
                onclick="document.getElementById('deleteModalSubject').classList.add('hidden'); document.getElementById('deleteModalSubject').classList.remove('flex');"
            ...
            <div class="flex justify-center items-center space-x-4">
                ...
                <!-- FORM DELETE -->
                <form id="deleteFormSubject" method="POST" action="" class="inline">
                    @csrf
                    @method('DELETE')
...
<script>
    function openDeleteSubject(id, name) {
        const modal = document.getElementById('deleteModalSubject');
        const nameField = document.getElementById('deleteSubjectName');
        const form = document.getElementById('deleteFormSubject');

        modal.classList.remove('hidden');
        modal.classList.add('flex');

        nameField.textContent = name;
        form.action = "/admin/subject/" + id;
    }
</script>
